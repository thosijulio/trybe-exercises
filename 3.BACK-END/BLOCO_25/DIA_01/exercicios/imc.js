...
const calcImc = () => {
  ...
  const getImcSituation = (imc) => {
    let response = '';
      ...
      else if (imc >= 18.5 && imc <= 24.9 ) response = 'Peso normal'
      else if (imc >= 25 && imc <= 29.9 ) response = 'Acima do peso (sobrepeso)'
      else if (imc >= 30 && imc <= 34.9 ) response = 'Obesidade grau I'
      else if (imc >= 35 && imc <= 39.9 ) response = 'Obesidade grau II'
